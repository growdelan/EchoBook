...
class OpenAITranslator:
    def __init__(self, api_key=None):
        ...
        if not self.api_key:
            raise ValueError(
                "Brak klucza API OpenAI. Ustaw zmienną środowiskową OPENAI_API_KEY lub przekaż klucz jako parametr."
            )
    ...
    def translate(self, text):
        if not text or text.strip() == "":
            return ""
        print(f"    – Długość tekstu: {len(text)} zn")
        try:
            ...
            print(f"    – Odpowiedź: {len(translated_text)} zn")
