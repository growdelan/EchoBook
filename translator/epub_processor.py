...
class EPUBProcessor:
    ...
    def load_epub(self):
        try:
            ...
            print(f"  - Plik EPUB wczytany pomyślnie")
            return True
    ...
    def extract_content(self):
        ...
        print(f"  - Zakończono ekstrakcję zawartości: znaleziono {document_count} dokumentów z {item_count} elementów")        
        return True
    ...
    def update_content_with_translation(self, translated_items):
        print("  - Rozpoczęcie aktualizacji zawartości EPUB przetłumaczonymi tekstami...")
        updated_count = 0
        ...
        for translated_item in translated_items:
            for item in self.content_items:
                if item['id'] == translated_item['id']:
                    ...
                    item['item'].set_content(translated_item['translated_html'].encode('utf-8'))
                    updated_count += 1
                    print(f"  - Zaktualizowano zawartość: {translated_item['href']}")
                    break
                    
        print(f"  - Zakończono aktualizację zawartości: zaktualizowano {updated_count} z {len(translated_items)} elementów")
                    
    def save_translated_epub(self, output_path):
        try:
            print(f"  - Zapisywanie przetłumaczonego pliku EPUB: {output_path}")
            epub.write_epub(output_path, self.book, {})
            print(f"  - Plik EPUB zapisany pomyślnie")
            return True
        except Exception as e:
