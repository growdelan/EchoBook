...
def parse_arguments():
    ...
    parser.add_argument('-o', '--output-dir', default='output', help='Katalog wyjściowy dla przetłumaczonych plików')
    parser.add_argument('-k', '--api-key', help='Klucz API OpenAI (opcjonalnie, domyślnie używa zmiennej środowiskowej OPENAI_API_KEY)')
...
def main():
    ...
    print(f"[1/6] Sprawdzanie pliku wejściowego...")
    ...
    print(f"[3/6] Ekstrakcja zawartości EPUB...")
    ...
    print(f"[4/6] Inicjalizacja tłumacza...")
    ...
    print(f"[5/6] Rozpoczęcie tłumaczenia zawartości...")
    translated_items = []
    ...
    for index, item in enumerate(processor.content_items, 1):
        ...
        if html_data['elements']:
            print(f"  - Znaleziono {elements_count} elementów tekstowych do tłumaczenia")
            ...
            print(f"  - Wysyłanie zapytania do API tłumaczenia...")
            ...
            if translated_combined:
                ...
                translated_html = text_splitter.update_html_with_translations(
                    html_data, 
                    translated_parts[:len(html_data['elements'])]
                )
                ...
                translated_items.append({
                    'id': item['id'],
                    'href': item['href'],
                    'translated_html': item['content']  # Zachowujemy oryginalny HTML
                })
        else:
            print(f"  - Brak tekstu do tłumaczenia, zachowanie oryginalnego HTML")
        ...
        print(f"  - Zakończono przetwarzanie elementu {index}/{total_items} (czas: {item_time:.2f}s)")
        print(f"  - Postęp: {index}/{total_items} ({(index/total_items*100):.1f}%)")
        print("------------------------------------------------")
    
    # Aktualizacja zawartości EPUB przetłumaczonymi tekstami
    print(f"[6/6] Aktualizacja zawartości EPUB przetłumaczonymi tekstami...")
    ...
    if processor.save_translated_epub(output_path):
        ...
        print(f"Całkowity czas wykonania: {total_time:.2f}s ({(total_time/60):.2f} minut)")
    else:
